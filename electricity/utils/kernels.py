...
## NOTE modified copy of gpytorch.kernels.periodic_kernel.PeriodicKernel to fix a bug.


print('coucou')
class MyCustomPeriodicKernel(Kernel):
    ...
    * :math:`\ell` is a lengthscale parameter.
    ...
    def __init__(self, active_dim, period_length_prior=None, period_length_constraint=None, **kwargs):
    ...
    def forward(self, x1, x2, diag=False, **params):
        print(x1.shape)
        active_dim = self._my_active_dim
        if active_dim: ## FIXME make this cleaner using e.g torch.narrow
          x1 = x1[:, [active_dim]]
          x2 = x2[:, [active_dim]]
        ...
        #diff = x1_ - x2_
        res = torch.sin(diff.mul(math.pi)).pow(2).mul(-2 / self.lengthscale ** 2).exp_()
        if diag:
            res = res.squeeze(0)
        return res
